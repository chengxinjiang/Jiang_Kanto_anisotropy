...
set infn1  = depth_0.5_vs.dat
set infn2  = depth_1_vs.dat
set infn3  = depth_2_vs.dat
set infn4  = depth_2.5_vs.dat

awk 'NR>1{if ($3==0.5 && $4!="NaN") print $1,$2,$4,$5}' $infn > $infn1
awk 'NR>1{if ($3==1.0 && $4!="NaN") print $1,$2,$4,$5}' $infn > $infn2
awk 'NR>1{if ($3==2.0 && $4!="NaN") print $1,$2,$4,$5}' $infn > $infn3
awk 'NR>1{if ($3==2.5 && $4!="NaN") print $1,$2,$4,$5}' $infn > $infn4
...
######### 0.5 km depth #########
# plot the depth of basement
set dmin = 0.6
set dmax = 0.8
makecpt -Cseis -T${dmin}/${dmax}/0.02 -Z -D > $cpt
...
psscale -C$cpt -D2.55/0.95/1.9/0.12 -Ba0.1f0.05 -O -K >> $outfn

########### 1.0 km depth ############
set dmin = 0.8
set dmax = 1.6
makecpt -Cseis -T${dmin}/${dmax}/0.05 -Z -D  > $cpt
...
awk -F, 'NR>1{print $4,$3}' station.lst | psxy -J -R -Sc0.03 -W1.5/100/100/100  -O -P -K >> $outfn
psscale -C$cpt -D2.55/0.95/1.9/0.12 -Ba0.2f0.1 -O -K >> $outfn
...
awk -F, 'NR>1{print $4,$3}' station.lst | psxy -J -R -Sc0.03 -W1.5/100/100/100  -O -P -K >> $outfn
psscale -C$cpt -D2.55/0.95/1.9/0.12 -Ba0.5f0.1 -O -K >> $outfn
...
awk -F, 'NR>1{print $4,$3}' station.lst | psxy -J -R -Sc0.03 -W1.5/100/100/100  -O -P -K >> $outfn
echo 1 | awk '{print "139.6 35.85";print "140.5 35.7"}' | psxy -J -R -W1.5p,255/255/255,5_2:0p -K -O -P -V >> $outfn
echo 1 | awk '{print "139.5 35.6";print "140.3 35.9"}' | psxy -J -R -W1.5p,255/255/255,5_2:0p -K -O -P -V >> $outfn
psscale -C$cpt -D2.55/0.95/1.9/0.12 -Ba0.5f0.1 -O >> $outfn
