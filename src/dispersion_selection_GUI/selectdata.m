function [pointslist,xselect,yselect] = selectdata1(sta,unc,varargin)
...
if ~iscell(xdata)
  ...
  % only get the data from the line geometry
  xdata = xdata{1}(:);
  ydata = ydata{1}(:);
  npoints = length(xdata);
  %{
...
while selectionflag
  ...
  % loop for additional operiations
  for count = 1:100
      hh = input(['\n\n-Your choice:' ...	%
    '\n  (1) wavetypes: 1 for Rayl; 2 for Love\n'...	%
    '\n  (2) mode number\n'...
    '\n  (3) interpolation and save data\n'...
    '\n  (4) exit\n']);
  
    % get parameters for saving the data
    if hh == 1
        nn = input('enter wave type\n');
        if nn == 1
            wtype = 'rayl';
        else
            wtype = 'love';
    ...
    elseif hh == 2
        nmod = input('enter mode number\n');
    elseif hh == 3
        tinc = 0.2;
        xnew = min(xselect):tinc:max(xselect);
        ynew = interp1(xselect,yselect,xnew);
        unew = interp1(xselect,unc(pointslist),xnew);

        % write into file
        fname = sprintf('%s_%s.dat',sta{1},wtype);
        if isfile(fname)
            fout = fopen(fname,'a+');
        else
            fout = fopen(fname,'w');
        ...
        for kk = 1:length(xnew)
            fprintf(fout,'%d %6.2f %6.3f %5.3f\n',nmod,xnew(kk),ynew(kk),unew(kk));
        end
        fclose(fout);
    elseif hh == 4
        break;
